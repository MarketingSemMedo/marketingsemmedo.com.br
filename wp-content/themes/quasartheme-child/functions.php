...
function quasar_after_setup() {
    ...
    // Retrieve the directory for the localization files for Child Theme
...
function quasar_enqueue_frontend_files(){
	...
	*/
	...
	//Buttons
	wp_enqueue_style('quasar-buttons',  get_stylesheet_directory_uri().'/css/buttons.css', '','', 'all');
	...
	if(xr_get_option('content_padding','') == '10px'){
		...
		add_filter('wp_head', 'xr_style_callback');
	}
...
function quasar_get_featured_image($echo=true, $featured_image_size = 'rockthemes_featured', $hover_active = true){
	...
	if(isset($rockthemes_advanced_details['video_iframe_code']) && !empty($rockthemes_advanced_details['video_iframe_code'])){
		...
		if($display_video_directly){
			$return .= quasar_embed_video($rockthemes_advanced_details['video_iframe_code']);
		}
	}
	...
	elseif(isset($rockthemes_advanced_details['extra_featured_images']) && 
		...
		$return .= rockthemes_make_swiperslider_shortcode($post->ID,$featured_image_size);
	}
	...
	elseif(wp_get_attachment_image(get_post_thumbnail_id($post->ID),$featured_image_size)){
		$return .= wp_get_attachment_image(get_post_thumbnail_id($post->ID),$featured_image_size);
		if($hover_active){
			$return .= quasar_hover_effect($post->ID,true);
		}
	}
	...
	$return .= '
		<div class="quasar-title-date-container row">
			...
			<div class="large-3 medium-3 small-3 columns">
				...
				<div class="date-area-container main-gradient">'.$date.'</div>
				<div class="clear"></div>
			</div>
		</div>
	';
...
function quasar_breadcrumb() {
	...
	*/
	...
	if (!is_home() && !is_front_page() || is_paged()) {
		...
		elseif(is_search()) {
			echo $before.__('Search Results For : '.get_search_query(), 'quasar').$after;
